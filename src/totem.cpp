...
void Totem::initialize(const char** args){
...
void Totem::print_users(){
    if(this->kids.empty() && this->adults.empty() && this->elders.empty()){
        system("clear");  
...
void Totem::print_events(){
    if(this->movieTheaters.empty() && this->puppetShows.empty() && this->clubs.empty() && this->conserts.empty()){
        system("clear");  
        throw DataNotLoadedException("Imprimir usuários e ventos", "Você não pode exibir usuários pois eles ainda não foram carregados!");
    } else {
        std::cout << "\n\nFilmes: " << std::endl;
        MovieTheater::print_movie_theaters(this->movieTheaters);        
        std::cout << "\n\nBoates: " << std::endl;
        Club::print_clubs(this->clubs);
        std::cout << "\n\nShows: " << std::endl;
        Consert::print_conserts(this->conserts);
    }
    int id;
...
int Totem::login(){
    ...
    if(this->kids.empty() && this->adults.empty() && this->elders.empty()){
        ...
        std::cin >> id;
        if(adults.find(id) == adults.end() && elders.find(id) == elders.end()){
            system("clear");  
            ...
            if(adults.find(id) == adults.end())
                std::cout << "\nBem vindo, " <<  elders[loggedId].get_name() << std::endl;     
            else
                std::cout << "\nBem vindo, " <<  adults[loggedId].get_name() << std::endl;     

        }
    }    
}

void Totem::get_ages(float *max, float *min, float *avarage){
...
void Totem::get_dependents(float *max, float *min, float *avarage){
...
void Totem::get_dependent_relations(){
    std::map<int, Elder>::iterator itr; 
    std::map<int, Adult>::iterator itr2; 
...
void Totem::get_event_relations(){
    std::map<int, Elder>::iterator itr; 
    std::map<int, Adult>::iterator itr2; 
    std::map<int, std::string>::iterator itr3;
    std::map<int, std::string> out;
...
void Totem::get_biggest_elder_amount(){
